...
@register_model
def nat_custom(**kwargs):
    model = NAT(depths=[2, 2, 2, 2], num_heads=[2, 2, 2, 2], embed_dim=24, mlp_ratio=3,
...
def nat_mini(pretrained=False, **kwargs):
    model = NAT(depths=[3, 4, 6, 5], num_heads=[2, 4, 8, 16], embed_dim=64, mlp_ratio=3,
                 drop_path_rate=0.2, kernel_size=7, **kwargs)
    if pretrained:
        load_from(model, 'nat_mini_1k', 'cuda')
    return model
...
def nat_tiny(pretrained=False, **kwargs):
    model = NAT(depths=[3, 4, 18, 5], num_heads=[2, 4, 8, 16], embed_dim=64, mlp_ratio=3,
                drop_path_rate=0.2, kernel_size=7, **kwargs)
    if pretrained:
        load_from(model, 'nat_tiny_1k', 'cuda')
    return model
...
def nat_small(pretrained=False, **kwargs):
    ...
    if pretrained:
        load_from(model, 'nat_small_1k', 'cuda')
    return model
...
def nat_base(pretrained=False, **kwargs):
    ...
    if pretrained:
        load_from(model, 'nat_base_1k', 'cuda')
    return model

def load_from(model, reg_name, device):
    url = model_urls[reg_name]
    device = torch.device(device)
    pretrained_dict = torch.hub.load_state_dict_from_url(url=url, map_location=device)
