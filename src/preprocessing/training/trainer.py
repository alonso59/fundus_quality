...
class Trainer:
    ...
    def fit(self, train_loader, val_loader, num_epochs, stop_value,save_imgs: bool=True):
        ...
        for epoch in range(num_epochs):
            ...
            self.logger.info(f"Epoch: {epoch+1}/{num_epochs} --lr:{curr_lr:.3e}")
    ...
    def __segmentation_metrics(self, y_pred, y):
        iou_fn = MIoU(ignore_background=False, activation='sigmoid', device=self.device)
        metrics = SegmentationMetrics(ignore_background=False, activation='sigmoid', average=True)
        # metrics
        iou = iou_fn(y_pred, y)
