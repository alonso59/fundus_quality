...
def get_images_masks(file):
    ...
    for i in range(OPL.shape[0]):
        annotation[INL[i], i, :] = [cm.hsv(norm(2), bytes=True)[0], cm.hsv(norm(2), bytes=True)[1], cm.hsv(norm(2), bytes=True)[2]]
        annotation[OPL[i], i, :] = [cm.hsv(norm(2), bytes=True)[0], cm.hsv(norm(2), bytes=True)[1], cm.hsv(norm(2), bytes=True)[2]]
        annotation[ELM[i], i, :] = [cm.hsv(norm(3), bytes=True)[0], cm.hsv(norm(3), bytes=True)[1], cm.hsv(norm(3), bytes=True)[2]]
        annotation[PR1[i], i, :] = [cm.hsv(norm(1), bytes=True)[0], cm.hsv(norm(1), bytes=True)[1], cm.hsv(norm(1), bytes=True)[2]]
        annotation[PR2[i], i, :] = [cm.hsv(norm(1), bytes=True)[0], cm.hsv(norm(1), bytes=True)[1], cm.hsv(norm(1), bytes=True)[2]]
        annotation[BM[i], i, :]  = [cm.hsv(norm(4), bytes=True)[0], cm.hsv(norm(4), bytes=True)[1], cm.hsv(norm(4), bytes=True)[2]]
...
def main():
    ...
    train, val = train_test_split(patients, train_size=0.8, shuffle=True)
    ...
    for t in tqdm(train):
        ...
        slicing(image=data, mask=mask, name=name,
                path_img=train_path_ipatches, path_msk=train_path_mpatches, size=128, shift=128)
    for v in tqdm(val):
        ...
        slicing(image=data, mask=mask, name=name,
                path_img=val_path_ipatches, path_msk=val_path_mpatches, size=128, shift=128)
